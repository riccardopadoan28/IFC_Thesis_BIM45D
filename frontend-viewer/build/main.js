...
async function sigmaLoader(url, ifcLoader){
  console.log("Loading IFC from URL:", url); // Debug
  const response = await fetch(url);
  const buffer = await response.arrayBuffer();
  const ifcModel = await ifcLoader.ifcManager.parse(buffer);
  ifcModels.push(ifcModel);
  window.scene.add(ifcModel);
...
async function loadURL(event) {
  if (!window.rendered) {
    ...
    console.log("Received URL from Streamlit:", url);
